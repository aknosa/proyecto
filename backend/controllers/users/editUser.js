...
async function editUser(req, res, next) {
  ...
  try {
    ...
    const { id } = req.params;
    const { email, name, birthdate, phoneNumber, location } = req.body;
    ...
    if (email !== currentUser[0].email) {
      ...
      try {
        await sendMail({
          ...
          content: `Para validar tu nuevo email en la app de Intercambio de Libros haz click aquí: ${validationURL}`,
        });
      } catch (error) {
      ...
      await connection.query(
        `
        UPDATE users 
        SET name=?, email=?, update_date=NOW(), last_auth_update=NOW(), active=false, registration_code=?, image=?, birthdate=?, phone_number=?, location=?
        WHERE id=?
      `,
        [
          name,
      ...
      await connection.query(
        `
      UPDATE users 
      SET name=?, email=?, image=?, birthdate=?, phone_number=?, location=?, update_date=NOW()
      WHERE id=?
    `,
        [name, email, savedFileName, birthdate, phoneNumber, location, id]
      );
